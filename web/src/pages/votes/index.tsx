...
const ElectionVotes = () => {
  ...
  const [voteData, setVoteData] = useState<VerifyElectionVoteResponse['data']>();
  ...
  const handleVerifySuccess = useCallback((voteInfo: VerifyElectionVoteResponse['data']) => {
    ...
    <Paper className='table-wrapper rounded max-[700px]:!grid-rows-[1fr_90px]'>
      <section className='relative pt-24 p-8 flex items-center flex-col gap-16 overflow-y-auto'>
        ...
        {getData.data.docs.map(vote => (
          <ElectionVoteBubble key={vote._id} vote={vote} onVerifySuccess={handleVerifySuccess} />
        ))}
      </section>
