...
const ResultsSchema = new Schema(
  ...
  { ...SUB_DOC_OPTIONS, toJSON: { virtuals: true }, toObject: { virtuals: true } }
);

ResultsSchema.virtual('totalVotes').get(function () {
  const totalVotes = this.options.reduce((sum, item) => sum + item.votes, 0);
  return totalVotes;
});
